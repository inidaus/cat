...
<div class="d-flex justify-content-between align-items-center mb-3">
  <h2 class="glass-title"><i class="fas fa-question me-2"></i> Bank Soal</h2>
  <div class="d-flex gap-2 flex-wrap">
    <a href="<?= base_url('admin/soal/create') ?>" class="btn btn-success btn-sm">
  ...
  <div class="search-filter-container">
    <form method="GET" action="<?= base_url('admin/soal') ?>">
      <div class="row g-3 align-items-end">
        <div class="col-md-4">
          ...
          <input type="text" id="search" name="search" class="form-control search-input"
                 placeholder="Cari pertanyaan, kategori, atau pembimbing..."
                 value="<?= esc($search) ?>">
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-2">
          <a href="<?= base_url('admin/soal') ?>" class="btn btn-outline-secondary w-100">
          ...
          <tr>
            ...
            <td class="text-center">
              <div class="btn-group" role="group">
                ...
                <a href="<?= base_url('admin/soal/edit/' . $s['id']) ?>" class="btn btn-warning btn-sm" title="Edit Soal">
        ...
        <div class="modal-body">
          <div class="alert alert-info">
            ...
            <ul class="mb-0">
              ...
              <li><strong>Pembimbing</strong>: Gunakan nama lengkap pembimbing yang sudah ada</li>
          ...
          <div class="mb-3">
            <label class="form-label">
              <i class="fas fa-users me-1"></i>Pembimbing Tersedia:
            </label>
            <div class="border rounded p-2" style="max-height: 100px; overflow-y: auto;">
              <?php foreach ($pembimbing as $p): ?>
                <span class="badge bg-success me-1 mb-1"><?= esc($p['nama_lengkap']) ?></span>
              <?php endforeach; ?>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <a href="<?= base_url('admin/soal/template') ?>" class="btn btn-outline-info me-auto" target="_blank">
...
<div class="modal fade" id="modalExport" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <form action="<?= base_url('admin/soal/export') ?>" method="get">
        <div class="modal-header">
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <div class="form-text">
              <i class="fas fa-info-circle me-1"></i>
              Export akan mengunduh semua soal dalam kategori yang dipilih dalam format Excel (.xlsx)
            </div>
          </div>
        </div>
        <div class="modal-footer">
...
function confirmDelete(id, pertanyaan) {
  if (confirm(`Yakin ingin menghapus soal "${pertanyaan}"?`)) {
    ...
    form.method = 'POST';
    form.action = `<?= base_url('admin/soal/delete/') ?>${id}`;
...
function previewSoal(id) {
  window.open(`<?= base_url('admin/soal/preview/') ?>${id}`, '_blank', 'width=800,height=600,scrollbars=yes');
}

// Duplicate Soal Function
function duplicateSoal(id) {
  if (confirm('Yakin ingin menduplikat soal ini?')) {
    window.location.href = `<?= base_url('admin/soal/duplicate/') ?>${id}`;
  }
}
